...
async def clean_tables(async_session_test):
    """cleandata in all tables defore running test function"""
    async with async_session_test() as session:
        async with session.begin():
            for table_for_cleaning in CLEAN_TABLES:
                query = "TRUNCATE TABLE {0:s};"
                await session.execute(query.format(table_for_cleaning))
...
@pytest.fixture
async def get_user_from_database(asyncpd_pool):

    async def get_user_from_database_by_uuid(user_id: str):
        async with asyncpd_pool.acquire() as connection:
